...
rfgramlist=basepath.format("gram{}/other/grams/{}.xlsx")
...
for pi,page in enumerate(pages): #各単語ごとにトライグラムの呼び出し
    ...
    for si,soutei in enumerate(titens):
        if gramnumber == 1:
            ...
            phlists[1]=[" ".join(grams) for di,grams in enumerate(list(gramDf[soutei].values)) if di%2==1 and list(grams).count("-9")==0]
            phlists[2]=[" ".join(grams) for di,grams in enumerate(list(gramDf[soutei].values)) if di%2==0 and list(grams).count("-9")==0]
