...
wfgram   = basepath.format("gram{}/grams/{}.xlsx")
...
for pi,page in enumerate(pages): #各単語ごとにトライグラムの呼び出し
    ...
    for si,soutei in enumerate(titens):
        if gramnumber == 1:
            newcvclist=[" ".join(grams) for di,grams in enumerate(list(gramDf[si+1].values)) if di%2==0 and list(grams).count("-9")==0]
            newvcvlist=[" ".join(grams) for di,grams in enumerate(list(gramDf[si+1].values)) if di%2==1 and list(grams).count("-9")==0]
        else:
            newcvclist=[" ".join(grams) for di,grams in enumerate(list(gramDf[si+1].values)) if di%2==1 and list(grams).count("-9")==0]
            newvcvlist=[" ".join(grams) for di,grams in enumerate(list(gramDf[si+1].values)) if di%2==0 and list(grams).count("-9")==0]
